...
from compatible_type_checking import Content
import rpyc
import threading
...
class BrokerService(rpyc.Service):
    def create_topic(self, id: str, topicname: str) -> str:
    ...
    def exposed_login(self, id: str, callback) -> bool:
        with infos_lock:
    ...
    def exposed_logout(self, id: str) -> bool:
        with infos_lock:
            if id in infos["users_logged"].keys():
    ...
    def exposed_list_topics(self) -> list:
        return [topic["id"] for topic in infos["topics"]]
    
    def exposed_list_subscribed_topics(self, id: str) -> list:
        return [topic["id"] for topic in infos["topics"] if id in topic["users_subscribed"]]

    def exposed_publish(self, id: str, topic: str, data: str) -> bool: 
        with infos_lock:
    ...
    def exposed_subscribe_to(self, id: str, topic: str) -> bool:
        with infos_lock:
            topic_info = next((t for t in infos["topics"] if t["id"] == topic), None)
            if not topic_info:
                return False
    ...
    def exposed_unsubscribe_to(self, id: str, topic: str) -> bool:
        with infos_lock:
            topic_info = next((t for t in infos["topics"] if t["id"] == topic), None)
            if not topic_info:
                return False
